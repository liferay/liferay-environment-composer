...
# Set a unique prefix — used to name the worktree for each test run
LEC_TEST_NAME_PREFIX="test-my-feature"

setup_file() {
    common_setup_file
    # Optional: set file-level variables (shared across all tests in this file)
}
...
| Hook | Runs | Use for |
|------|------|---------|
| `setup_file` | Once, before all tests in a file | Setting environment variables shared across tests |
...
Call this in `setup`. Reads `LEC_TEST_NAME_PREFIX` (which you must set at file scope) and:
1. Runs `lec init` to create a fresh git worktree
2. Sets `TEST_WORKSPACE_DIR` to the new worktree path
...
Call this in `teardown`. Cleans up the worktree created by `common_setup`:
1. `docker compose down -v` — stops containers and removes volumes
2. `_lec fn _clean` — cleans generated files
3. `_lec fn _removeWorktree` — removes the git worktree
...
Returns a Unix timestamp. Used to generate unique worktree names.
...
Docker Compose assigns ports from ranges defined in `ports.env`. Never hardcode port numbers in assertions. Use `docker compose port` to look up the published host port for a service:

```bash
# Returns just the port number (strips the host prefix)
local port
port=$(docker compose port liferay 8080 | sed 's,^.*:,,g')
